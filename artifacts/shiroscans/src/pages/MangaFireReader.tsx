...
export default function MangaFireReaderPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/api/mangafire/read/${encodeURIComponent(safeChapterId)}?slug=${encodeURIComponent(safeSlug)}`)
      ...
      <div className={`fixed top-0 left-0 right-0 z-50 bg-[#0A0A0F]/97 backdrop-blur-md border-b border-white/[0.06] flex items-center h-12 px-3 gap-2 ${barCls}`}>
        ...
        <div className="flex-1 min-w-0 text-center">
          <p className="text-xs text-white/80 font-semibold truncate leading-tight">
            {safeSlug.split(".")[0]?.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()) ?? "Reading"}
          </p>
          <p className="text-[11px] text-primary leading-tight">
            {currentChapter ? `Chapter ${currentChapter.number}` : `Chapter ${safeChapterId}`}
          </p>
        </div>
