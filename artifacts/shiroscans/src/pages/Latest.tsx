...
import { useGetLatestManga } from "@workspace/api-client-react";
...
export default function LatestPage() {
  ...
  const { data: mdxData, isLoading: mdxLoading } = useGetLatestManga(
    { page, provider, type: undefined, status: undefined, genre: undefined },
    {
      query: {
        queryKey: ["latest", page, provider] as const,
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/api/asurascans/latest?page=${page}`)
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-black text-white tracking-tight">Latest Updates</h1>
            <p className="text-sm text-white/40 mt-0.5">Fresh chapters from AsuraScans</p>
          </div>
