...
type Provider = "mangadex" | "weebcentral" | "comix";

const PROVIDERS: { id: Provider; label: string }[] = [
  { id: "mangadex", label: "MangaDex" },
  { id: "weebcentral", label: "WeebCentral" },
  { id: "comix", label: "Comick" },
];
...
export default function RankingsPage() {
  ...
  const { data, isLoading } = useGetPopularManga(
    { provider, page: 1 },
    { query: { queryKey: ["rankings", provider], staleTime: 300_000 } }
  );
    ...
    <div className="bg-[#07070d] min-h-screen">
      ...
      <div className="bg-gradient-to-b from-amber-500/10 to-transparent px-4 pt-6 pb-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-xl font-black text-white tracking-tight">Comics Ranking</h1>
        ...
        {isLoading ? (
          <div className="space-y-4">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="flex items-center gap-3 p-3">
                ...
                <Skeleton className="w-10 h-5 bg-[#1a1a2e] rounded" />
              </div>
            ))}
          </div>
        ) : (
          <div className="space-y-1">
            {items.map((item, idx) => {
              const rank = idx + 1;
              const rowBg =
                rank <= 3 ? "bg-amber-500/[0.04] hover:bg-amber-500/[0.08]" :
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {item.status && (
                        <span className="text-[10px] text-white/25">{item.status}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="shrink-0 text-right">
                    ...
                    {rank <= 3 && (
                      <div className="text-[9px] text-amber-400/50 mt-0.5 font-bold">
                        #{rank}
                      </div>
                    )}
                  </div>
